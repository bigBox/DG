...
@ccclass
export default class UIUnionOther extends UIBase {
    @property({ type: cc.Label, displayName: "lblName", tooltip: "标题" })
    lblName: cc.Label = null;

    @property({ type: cc.Label, displayName: "lblOutNote", tooltip: "对外商会公告" })
    ...
    unionclick(event, eventData) {
        let self = this;
        global.Instance.MsgPools.send('guildList', { guildId: this.data.id }, function (msg) {
            global.Instance.Log.debug('guildList',msg)
            switch (eventData) {
                case "商会信息":
    ...
    */
    ...
    btnClose() {
        ...
        global.Manager.UIManager.close('UIUnionOther');
    };
    // update (dt) {}
}
