...
@ccclass
export default class UICatchGoods extends cc.Component {
    @property({ type: cc.Node, displayName: "ndNext", tooltip: "ndNext" })
    ndNext: cc.Node = null;
    @property({ type: cc.Node, displayName: "btnClose", tooltip: "退出小游戏游戏" })
    btnClose: cc.Node = null;
    @property({ type: cc.Node, displayName: "btnPuase", tooltip: "停止当前进行的游戏" })
    btnPuase: cc.Node = null;
    ...
    beginGame() {
        ...
        global.Module.CatchGoodsData.clearNeeds();
        this.createFlylist();
        this.resetPan();
    };
    refBlood(){
    ...
    //游戏关闭按钮
    setInGame(isInGame) {
        this.btnPuase.active = isInGame;
        this.btnClose.active = !isInGame;
    ...
    //分数
    reflashScore() {
        let score = global.Module.CatchGoodsData.getScore();
    ...
    //左侧
    reflashMakeItem() {
        let dropItems = global.Module.CatchGoodsData.getFlyItems();
        for (let i = 0; i < 3; i++) {
            ...
            global.CommonClass.Functions.setTexture(spNext, picPath, function (image) {
                spNext.scale = global.CommonClass.Functions.getToscale(spNext,110,80);
            }.bind(this));
        }
    };
    ...
    createDropitem() {
        ...
        let dropItem = global.Module.CatchGoodsData.popDropItem();
        if (dropItem) {
            ...
            global.Module.CatchGoodsData.removeFlyItem(dropItem.ID);
            this.reflashMakeItem();
        }
    }
    selfUpdate(dt) {
        ...
        for (let index in ndDrops) {
            ...
            if (dropItem) {
                if (this.checkIsCatch(dropItem)) {
                    ...
                    let oldMakeID = global.Module.CatchGoodsData.getMakeID();

                    global.Instance.MsgPools.send('meetDrop', data, function (msg) {
                        global.Instance.Log.debug('接鸡蛋小游戏返回  meetDrop', msg)
                        if (msg.errorID == 0) {
                            if (msg.makeID > 0) {
                                self.playSmoke();
    ...
    showDlg(isShow, data) {
        let self = this;
        if (isShow) {
            global.Manager.UIManager.open('DlgOver', null, function (panel) {
                panel.show(data, function () {
                    let ndRestart = self.node.getChildByName('ndRestart');
                    ndRestart.active = true;
                    self.btnClose.active = true;
                });
            });
        } else {
    ...
    startGame() {
        ...
        let data = { positionX: posX, buildID: 7104 };

        let self = this;
        global.Instance.MsgPools.send('startMeetEgg', data, function (msg) {
         
            if (msg.errorID == 0) {
                global.Module.CatchGoodsData.start(self);
