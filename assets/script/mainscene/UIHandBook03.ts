...
@ccclass
export default class UIHandBook03 extends UIBase {
    ...
    // 原型
    onClickPrototype() {
        global.Manager.UIManager.open('UIHandBook05', null, function (panel) {
            if (panel != null) {
                let book = this.items[this.curIdx];
    ...
    touchEvent(event) {
        ...
        } else if (event.type == cc.Node.EventType.TOUCH_MOVE) { } else if (event.type == cc.Node.EventType.TOUCH_END || event.type == cc.Node.EventType.TOUCH_CANCEL) {
            if (!this.isInChagne) {
    ...
    btnClose(event, arg) {
        global.Manager.UIManager.close('UIHandBook03');
    };
}
