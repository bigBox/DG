...
export default class FarmItem extends cc.Component {
    ...
    static create(itemData, itemIdx, parent, position, callback) {
            let filePath =  "prefab/component/FarmItem";
            cc.loader.loadRes(filePath, function (err, prefab) 
            {
                if (err==null)
                {
                    ...
                    let itemIcon = newNode.getComponent(FarmItem);
    ...
    setItem(itemData) {
        ...
        if (itemData != null)
        {
            ...
            let data=global.Manager.DBManager.getItemNew(itemData.matrial);   
            if (data != null)
    ...
    onFarmItemClick(msg) {
        let evt = new cc.Event.EventCustom('onFarmItemClick', true);
        let itemClass = this.node.getComponent(FarmItem);
